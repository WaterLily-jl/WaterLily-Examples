...
function a_wing(; L=32, aoa=15, Ay=0, taper=0.75, sweep=15, span=1.0, center=(0.20, 0.50), Re=1e4, domain=(8,4,2.0), U=1, T=Float32, mem=Array)
    ...
    finite_span = SA{T}[10, 10]
    foil = interpNurbs(pnts; p=3) # interpolate a closed airfoil curve

    function extrude(x, t)
        x[3] > b && return finite_span # if outside the span then no body
        chord_scale = 1 / ((taper - 1) * (x[3] / b) + 1) # scaling due to taper ratio
        # perform transformation of taper and sweep as wing is extruded
        return chord_scale .* SA[x[1] - (xc * L * n + tan(Λ) * (x[3])), x[2] - (yc * L * m) + ampy * sin(ω*t)]
    end

    body = ParametricBody(foil; map=extrude, ndims=3)
    Simulation(round.(Int, (n * L, m * L, l * L)), (U*cos(α), U*sin(α), T(0)), L;
         U, exitBC=true, ν=T(U*L/Re), body, T, mem)
end

# Parameters
T = Float32
mem = Array # using CUDA; mem=CuArray # to run on GPU
Re = 5e4 # Reynolds number
L = 24 # length of chord in points
...
U = 1 # velocity
Ay = 0 # 4 # amplitude of "heaving" motion in points
...
viz!(sim; duration=sim_time(sim)+10,
    algorithm=:iso, isovalue=0.25, colormap=[:lightblue], alpha=0.9, transparency=true,
    body_color=(:grey, 0.9), body2mesh=false, hidedecorations=true, azimuth, elevation,
    remeasure=false, # use remeasure=true if heaving wing
    video="ThreeD_AsymmetricalWing.mp4", # remove `video`for co-visualization during runtime
)